...
function SingleProductCard({ title, name, imgURL, previousPrice, salePrice, category, rating, desc }) {
    const stars = [];
    for (let i = 1; i <= 5; i++) {
        stars.push(<svg xmlns="http://www.w3.org/2000/svg" fill={i < rating ? "currentColor" : "none"} viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6 text-yellow-300">
    ...
    return (
        <div
            className="max-w-sm hover:shadow-xl transition-shadow mx-auto p-0 rounded-3xl border-2 overflow-hidden">
            <img className='h-72 object-cover p-0 w-screen' src={imgURL} alt={title} />
            <div className="p-6">
                <Link to={'*'}>
